...
function App() {
  ...
  return (
    <>
      <div id='body' style={{ width: '100%', height: '100vh' }} className='flex flex-col p-5 justify-center items-center'>
        ...
        <div className='flex flex-col justify-center w-4/5 h-1/2 border-double border-4 border-indigo-600  items-center static'>
          ...
          <input className='w-1/2 h-14  rounded-full' type="text" value={newItem} onChange={(e) => setNewItem(e.target.value)} />
          <button onClick={AddTodo} className='btn   bg-white'><i className="fa-solid fa-plus absolute top-1/2 mt-3 rounded-full w-9 h-9 text-center font-bold text-xl font-bold bg-red-500 text-gray-50 right-1/3 mb-1 ml-9  "></i></button>
        </div>
        <ul>
          {
            todo.length > 0 ? (
              <>
                ...
                (
                  ...
                  <li style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', margin: '20px', width: '30rem', marginLeft: '40px', border: '2px solid violet', borderRadius: '10px' }} key={item.id} >{index + 1}  . {item.value}
                    <div>
                     ...
                     ( <>
                        ...
                        <button onClick={() => startEdit(item)} className='p-2' style={{ fontSize: '20px', cursor: 'pointer' }} >
                          <i style={{ color: 'blue' }} className="fa-solid fa-pencil"></i>
                        </button>
                        <button style={{ fontSize: '20px', cursor: 'pointer' }} onClick={(id) => deleteTodo(item.id)} ><i style={{ color: 'red', marginLeft: 'auto' }} className="fa-solid fa-trash"></i></button>
                      </>)}
